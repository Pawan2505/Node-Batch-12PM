...
exports.extraCategory = async (req, res) => {
  try {
    ...
    return res.redirect("/extracategory/add_extracategory");
  } catch (error) {
    console.log(error);
    req.flash("error", "Somthing Wrong!!!");
    return res.redirect("back");
...
exports.viewExtraCategory = async (req, res) => {
  try {
    ...
    return res.render("view_extracategory", { extraCategories });
  } catch (error) {
    console.log(error);
    req.flash("error", "Somthing Wrong!!!");
    return res.redirect("back");
...
exports.editExtraCategory = async (req, res) => {
  try {
    ...
    if(extraCategory){
      let categories = await Category.find();
    let subCategories = await SubCategory.find();
      return res.render("edit_extraCategory", {extraCategory, categories, subCategories});
    }else{
      req.flash("error", "Extra Category not Found");
      return res.redirect("back");
    }
